...
namespace BioIK {
	...
	public class Evolution {		
        ...
        public double[] iniSolution;
        private bool temp = false;
        public bool justonce = false;
        private bool Killed = false;
        public double[] refSSolution = null;
        public double[] refSSolutionref;
        //Initialises the algorithm
        public Evolution(Model model, int populationSize, int elites, bool useThreading, double[] refSolution) {
            //Debug.Log("Usethread:" + useThreading);
            //Debug.Log("Constructer initialize");
            Model = model;
            ...
            iniSolution = new double[Dimensionality]; 
            ...
            for(int i=0; i<Elites; i++) {
                ...
                //     Debug.Log("refsol beforeloss 90  = " + String.Join("",
                //new List<double>(refSolution)
                //.ConvertAll(j => j.ToString())
                //.ToArray()));
                refSSolutionref = new double[refSolution.Length];
                for (int j = 0; j < refSolution.Length; j++)
                {
                    refSolution[j] = refSSolutionref[j];
                }
                Optimisers[index] = new BFGS(Dimensionality, (x, refSSolution) => Models[index].ComputeLoss(x,refSSolution), (y, refSSolution) => Models[index].ComputeGradient(y, refSSolution, 1e-5 )); //objectivefunctionupdaterequired
                //Optimisers[index] = new BFGS(Dimensionality, x => Models[index].ComputeLoss(x), y => Models[index].ComputeGradient(y, 1e-5));
           //       Debug.Log("refsol BFGS = " + String.Join("",
           //new List<double>(refSolution)
           //.ConvertAll(j => j.ToString())
           //.ToArray()));
            }
            ...
            if(UseThreading) {
                ...
                for(int i=0; i<Elites; i++) {
                    ...
                    Threads[index] = new Thread(x => SurviveThread(index,refSolution));
                    //Threads[index].Start();
                }
            } else {
        ...
        public double[] Optimise(int generations, double[] seed,double[] refSolution)
        {
            //if (justonce)
            //{
            //    for (int i = 0; i < Elites; i++)
            //    {
            //        Threads[i] = new Thread(x => SurviveThread(i, refSolution));

            //    }
            //    justonce = true;
            //}
            refSSolution = new double[refSolution.Length];
            for (int i = 0; i < refSolution.Length; i++)
            {
                refSSolution[i] = refSolution[i];
            }

            if (UseThreading) {
                for(int i=0; i<Elites; i++) {
                    //Threads[i] = new Thread(x => SurviveThread(i, refSolution));
                    if (!Threads[i].IsAlive) { // This will basically start survive thread()
                        Threads[i].Start();
                    }
                }
            }
            ...
            //Fitness = Model.ComputeLoss(Solution); ////objectivefunctionupdaterequired
            //Debug.Log("refsol beforeloss 175  = " + String.Join("",
            // new List<double>(refSSolution)
            // .ConvertAll(i => i.ToString())
            // .ToArray()));
            Fitness = Model.ComputeLoss(Solution,refSSolution);
            //Debug.Log("initial Loss :" + Fitness);



            if (!Model.CheckConvergence(Solution)) {
                //Initialise(seed);
                Initialise(seed, refSolution);
                for (int i=0; i<Elites; i++) {
				...
				for(int i=0; i<generations; i++) {
					...
					//Evolve();               // This is where the solutions are optimized
                    Evolve(refSolution);               // This is where the solutions are optimized
                }
			}
            //finalFitness = Model.ComputeLoss(Solution, refSolution);
            //Debug.Log("final Loss :" + finalFitness);

            //cosanglescore = Score(iniSolution, Solution);
            //Debug.Log("Cosine angle score:" +cosanglescore);
            //if (Model.Character.cosloss == false)
            //{
            //    Model.finalfinal = true;
            //    Model.ComputeLoss(Solution, refSolution);
            //    Model.finalfinal = false;
            //}
            return Solution;
		}

        public double Score(double[] vectorArray1, double[] vectorArray2)
        {
        ...
        private void Initialise(double[] seed, double[] refsolution) {
			for(int i=0; i<Dimensionality; i++) {
				Population[0].Genes[i] = seed[i];
				Population[0].Momentum[i] = 0.0;
			}
            //Population[0].Fitness = Model.ComputeLoss(Population[0].Genes, refsolution);//objectivefunctionupdaterequired
            //Debug.Log("refsol beforeloss 296  = " + String.Join("",
            // new List<double>(refSSolution)
            // .ConvertAll(i => i.ToString())
            // .ToArray()));
            Population[0].Fitness = Model.ComputeLoss(Population[0].Genes,refSSolution); //objectivefunctionupdaterequired
            //Debug.Log("Population[0] Fitness:" + Population[0].Fitness);

			for(int i=1; i<PopulationSize; i++) {
				//Reroll(Population[i]);  // Reroll is the function where random initial joint angles are initialized
                 Reroll(Population[i],refsolution);
            }
		...
		private void Evolve(double[] refsolution) {
			//Create mating pool
			Pool.Clear();
            ...
            if(UseThreading) {
                ...
                for(int i=0; i<Elites; i++) { // Evolution of elite individuals takes place simultaneously on another thread
                    Handles[i].Set();
                }
                ...
                for(int i=Elites; i<PopulationSize; i++) {
                    if(PoolCount > 0) {
                        ...
                        Reproduce(Offspring[i], parentA, parentB, prototype,refsolution);
                        ...
                        //Fill the population
                        //Reroll(Offspring[i]);
                        Reroll(Offspring[i],refsolution);
                    }
                }
                ...
                for(int i=Elites; i<PopulationSize; i++) {
                    if(PoolCount > 0) {
                        Individual parentA = Select(Pool); // select is where the rank based selection criteria is used
                        Individual parentB = Select(Pool);
                        Individual prototype = Select(Pool);
                ...
                for(int i=0; i<Elites; i++) {
                    ...
                    SurviveSequential(i, duration,refsolution);
                }
            }
			...
			if(!TryUpdateSolution() && !HasAnyEliteImproved()) {
				Initialise(Solution,refsolution);
			} else {
                ComputeExtinctions();
            }
		}
        ...
        private void SurviveThread(int index,double[] refsolution)
         
        {  //Debug.Log("Survive thread");
     //       Debug.Log("refsol survive thread = " + String.Join("",
     //new List<double>(refsolution)
     //.ConvertAll(j => j.ToString())
     //.ToArray()));
            ...
            while (ThreadsRunning) {
                ...
                //Exploit
             //   Debug.Log("refsol beforeloss 511  = " + String.Join("",
             //new List<double>(refSSolution)
             //.ConvertAll(i => i.ToString())
             //.ToArray()));
                double fitness = Models[index].ComputeLoss(elite.Genes,refSSolution); //objectivefunctionupdaterequired
                Optimisers[index].Minimise(elite.Genes, refSSolution, ref Evolving);
		...
		private void SurviveSequential(int index, double timeout, double[] refsolution) {
            //Copy elitist survivor
            Individual survivor = Population[index];
            Individual elite = Offspring[index];
            for(int i=0; i<Dimensionality; i++) {
            ...
            //Exploit
            //double fitness = Models[index].ComputeLoss(elite.Genes, refsolution);
            //Debug.Log("refsol beforeloss 549  = " + String.Join("",
            // new List<double>(refSSolution)
            // .ConvertAll(i => i.ToString())
            // .ToArray()));
            double fitness = Models[index].ComputeLoss(elite.Genes, refSSolution); //objectivefunctionupdaterequired
            Optimisers[index].Minimise(elite.Genes, refSSolution, timeout);
            if(Optimisers[index].Value < fitness) {
                for(int i=0; i<Dimensionality; i++) {
                    elite.Momentum[i] = Optimisers[index].Solution[i] - elite.Genes[i];
                    elite.Genes[i] = Optimisers[index].Solution[i];
        ...
        private void Reproduce(Individual offspring, Individual parentA, Individual parentB, Individual prototype,double[] refsolution)
        {
			...
			for(int i=0; i<Dimensionality; i++) {
				...
				double momentum = UnityEngine.Random.value * parentA.Momentum[i] + UnityEngine.Random.value * parentB.Momentum[i];
                ...
                if(Constrained[i]) {
                    if(UnityEngine.Random.value < mutationProbability) {
                        offspring.Genes[i] += (UnityEngine.Random.value * (UpperBounds[i] - LowerBounds[i]) + LowerBounds[i]) * mutationStrength;
                    }
                }
				...
				Weight = UnityEngine.Random.value;
				offspring.Genes[i] += 
					Weight * UnityEngine.Random.value * (0.5 * (parentA.Genes[i] + parentB.Genes[i]) - offspring.Genes[i])
					+ (1.0-Weight) * UnityEngine.Random.value * (prototype.Genes[i] - offspring.Genes[i]);
				...
				//Momentum
				offspring.Momentum[i] = UnityEngine.Random.value * momentum + (offspring.Genes[i] - Gene);
			}

            //Fitness
            //offspring.Fitness = Model.ComputeLoss(offspring.Genes); //objectivefunctionupdaterequired
            //cosanglescore = Score(offspring.Genes, offspring.Genes);
            //Debug.Log("refsol beforeloss 614  = " + String.Join("",
            //new List<double>(refSSolution)
            //.ConvertAll(i => i.ToString())
            //.ToArray()));
            offspring.Fitness = Model.ComputeLoss(offspring.Genes,refSSolution); //objectivefunctionupdaterequired
        }

        //Generates a random individual
        //private void Reroll(Individual individual) {
        private void Reroll(Individual individual,double[] refsolution)
        {
            for (int i=0; i<Dimensionality; i++) {
				if(Constrained[i]) {
					individual.Genes[i] = (double)UnityEngine.Random.Range((float)LowerBounds[i], (float)UpperBounds[i]);
                    //iniSolution[i] = individual.Genes[i];
            ...
            //Debug.Log("refsol beforeloss 636  = " + String.Join("",
            //new List<double>(refSSolution)
            //.ConvertAll(i => i.ToString())
            //.ToArray()));
            individual.Fitness = Model.ComputeLoss(individual.Genes,refSSolution);  //objectivefunctionupdaterequired
        }
    ...
    public partial class BFGS {
        ...
        public System.Func<double[],double[], double> Function;
        public System.Func<double[], double[],double[]> Gradient;
        ...
        public BFGS(int dimensionality, System.Func<double[],double[], double> function, System.Func<double[],double[], double[]> gradient)
        {
            //Debug.Log("BFGS");
        ...
        public void Minimise(double[] values, double[] refsolution, ref bool evolving)
        {
            for (int i=0; i<Dimensionality; i++) {
                Solution[i] = values[i];
            }
            ...
            while(evolving) {
                ...
                if(_Task == Task.FG_LN || _Task == Task.FG_ST) {
                    ...
                    NewF = Function(Solution,refsolution);
                    NewG = Gradient(Solution,refsolution);
                    F = NewF;
                    for(int i=0; i<Dimensionality; i++) {
                        G[i] = NewG[i];
                    }
            ...
            Value = Function(Solution,refsolution);
        }

        public void Minimise(double[] values,double[] refsolution, double timeout) {
            for(int i=0; i<Dimensionality; i++) {
                Solution[i] = values[i];
            }
            ...
            System.DateTime timestamp = Utility.GetTimestamp();
            while(Utility.GetElapsedTime(timestamp) < timeout) {    //maybe where time criteria used
                setulb(
                    Dimensionality, Corrections, Solution, 0, LowerBounds, 0, UpperBounds, 0, NBD, 0, ref F, G, 0,
                    Factor, Tolerance, Work, 0, IWA, 0, ref _Task, IPrint, ref _CSave,
                    LSave, 0, ISave, 0, DSave, 0
